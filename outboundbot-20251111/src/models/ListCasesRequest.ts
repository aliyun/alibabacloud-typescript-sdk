...
export class ListCasesRequest extends $dara.Model {
  ...
  validate() {
    if(Array.isArray(this.caseIds)) {
      $dara.Model.validateArray(this.caseIds);
    }
    ...
    if(this.labelSearch) {
      $dara.Model.validateMap(this.labelSearch);
    }
