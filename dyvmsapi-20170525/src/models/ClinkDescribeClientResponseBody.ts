// This file is auto-generated, don't edit it
import * as $dara from '@darabonba/typescript';


export class ClinkDescribeClientResponseBodyDataClientClidArea extends $dara.Model {
  /**
   * @remarks
   * 地区组名称
   * 
   * @example
   * 示例值示例值
   */
  areaGroupName?: string;
  /**
   * @remarks
   * 号码类型，默认值为0，0: 号码；1: 动态号码组
   * 
   * @example
   * 0
   */
  assignType?: number;
  /**
   * @remarks
   * 动态号码组名称，当assignType（号码类型）值为1（动态号码组）时为必填项
   * 
   * @example
   * xxx
   */
  dynamicTelGroupName?: string;
  /**
   * @remarks
   * 外显号码
   */
  obClids?: string[];
  static names(): { [key: string]: string } {
    return {
      areaGroupName: 'AreaGroupName',
      assignType: 'AssignType',
      dynamicTelGroupName: 'DynamicTelGroupName',
      obClids: 'ObClids',
    };
  }

  static types(): { [key: string]: any } {
    return {
      areaGroupName: 'string',
      assignType: 'number',
      dynamicTelGroupName: 'string',
      obClids: { 'type': 'array', 'itemType': 'string' },
    };
  }

  validate() {
    if(Array.isArray(this.obClids)) {
      $dara.Model.validateArray(this.obClids);
    }
    super.validate();
  }

  constructor(map?: { [key: string]: any }) {
    super(map);
  }
}

export class ClinkDescribeClientResponseBodyDataClientPermission extends $dara.Model {
  /**
   * @remarks
   * 语音转写，0: 关闭；1: 呼入开启；2 外呼开启；3 全部开启；默认值为 0
   * 
   * @example
   * 1
   */
  asr?: number;
  /**
   * @remarks
   * 外呼权限，0: 关闭；1: 无限制；2: 国内长途；3: 国内本地，默认值为 1
   * 
   * @example
   * 1
   */
  call?: number;
  /**
   * @remarks
   * 通话记录查看权限，1: 全部；2: 所属队列；3:本座席，默认值为 1
   * 
   * @example
   * 1
   */
  cdr?: number;
  /**
   * @remarks
   * 在线客服查看会话记录权限 ，0：全部、1：所属队列、2：本座席； 默认值为 0
   * 
   * @example
   * 1
   */
  chat?: number;
  /**
   * @remarks
   * 其他数据查看权限：1：全部 2：所属员工组 3：自己 4:指定员工组 ,默认所属员工组
   * 
   * @example
   * 1
   */
  otherData?: number;
  /**
   * @remarks
   * 通话录音权限，0: 关闭；1: 呼入；2: 外呼；3: 全部，默认值为 3
   * 
   * @example
   * 1
   */
  record?: number;
  /**
   * @remarks
   * 录音试听下载权限，0: 关闭；1: 试听下载；2：试听，默认值 1
   * 
   * @example
   * 1
   */
  recordDownload?: number;
  /**
   * @remarks
   * 短信发送权限，0: 关闭；1: 开启，默认值为 0
   * 
   * @example
   * 1
   */
  sms?: number;
  /**
   * @remarks
   * 外呼任务查看权限，1：全部 3：自己，默认全部
   * 
   * @example
   * 1
   */
  taskInventory?: number;
  /**
   * @remarks
   * 通话转移/咨询权限，可选范围，0：全部，1：所属员工组, 默认所属员工组
   * 
   * @example
   * 1
   */
  transfer?: number;
  static names(): { [key: string]: string } {
    return {
      asr: 'Asr',
      call: 'Call',
      cdr: 'Cdr',
      chat: 'Chat',
      otherData: 'OtherData',
      record: 'Record',
      recordDownload: 'RecordDownload',
      sms: 'Sms',
      taskInventory: 'TaskInventory',
      transfer: 'Transfer',
    };
  }

  static types(): { [key: string]: any } {
    return {
      asr: 'number',
      call: 'number',
      cdr: 'number',
      chat: 'number',
      otherData: 'number',
      record: 'number',
      recordDownload: 'number',
      sms: 'number',
      taskInventory: 'number',
      transfer: 'number',
    };
  }

  validate() {
    super.validate();
  }

  constructor(map?: { [key: string]: any }) {
    super(map);
  }
}

export class ClinkDescribeClientResponseBodyDataClientQueues extends $dara.Model {
  /**
   * @remarks
   * 队列名称
   * 
   * @example
   * xx1
   */
  name?: string;
  /**
   * @remarks
   * 优先级
   * 
   * @example
   * 8
   */
  penalty?: number;
  /**
   * @remarks
   * 队列号
   * 
   * @example
   * 1122
   */
  qno?: string;
  static names(): { [key: string]: string } {
    return {
      name: 'Name',
      penalty: 'Penalty',
      qno: 'Qno',
    };
  }

  static types(): { [key: string]: any } {
    return {
      name: 'string',
      penalty: 'number',
      qno: 'string',
    };
  }

  validate() {
    super.validate();
  }

  constructor(map?: { [key: string]: any }) {
    super(map);
  }
}

export class ClinkDescribeClientResponseBodyDataClient extends $dara.Model {
  /**
   * @remarks
   * 是否激活，0: 否；1: 是
   * 
   * @example
   * 1
   */
  active?: number;
  /**
   * @remarks
   * 区号
   * 
   * @example
   * 010
   */
  areaCode?: string;
  /**
   * @remarks
   * 座席满意度自动执行，1：开启；0：关闭，默认开启
   * 
   * @example
   * 1
   */
  autoInvestigation?: number;
  /**
   * @remarks
   * 座席绑定电话
   * 
   * @example
   * xxx
   */
  bindTel?: string;
  /**
   * @remarks
   * 外显号码数组
   */
  clid?: string[];
  /**
   * @remarks
   * 按地区外显配置，当clidType（外显号码类型）值为3且clidRule（外显规则）值为2（自定义）时有意义，具体配置项见 clidArea 对象
   */
  clidArea?: ClinkDescribeClientResponseBodyDataClientClidArea[];
  /**
   * @remarks
   * 外显规则 当clidType（外显号码类型）值为0（全部）时，1:随机；2: 轮选。 当clidType（外显号码类型）值为3（智能外显）时，1:动态；2: 自定义。
   * 
   * @example
   * 1
   */
  clidRule?: number;
  /**
   * @remarks
   * 外显号码类型，0: 全部; 2: 座席指定; 3: 智能外显
   * 
   * @example
   * 1
   */
  clidType?: number;
  /**
   * @remarks
   * 云号码外呼开关，0-关，1-开
   * 
   * @example
   * 0
   */
  cloudNumberEnabled?: number;
  /**
   * @remarks
   * 云号码四种呼叫模式；数组长度 为 4，依次对应云号码外呼的四种模式（实体卡、工作卡、两端呼、RTC)的开启状态；
   */
  cloudNumberModes?: number[];
  /**
   * @remarks
   * 座席号
   * 
   * @example
   * 1111
   */
  cno?: string;
  /**
   * @remarks
   * 座席crm id
   * 
   * @example
   * 123
   */
  crmId?: string;
  /**
   * @remarks
   * 号码隐藏类型，0: 不隐藏；1: 隐藏规则与全局设置保持一致
   * 
   * @example
   * 0
   */
  hiddenTel?: string;
  /**
   * @remarks
   * 呼入整理时长，客户来电座席接听，双方通话结束后座席的话后处理时长，此期间座席不接收新的客户来电。取值范围：0～3600秒。
   * 
   * @example
   * 20
   */
  ibWrapupTime?: number;
  /**
   * @remarks
   * 呼入整理类型，1：队列；2：座席。
   * 
   * @example
   * 1
   */
  ibWrapupType?: number;
  /**
   * @remarks
   * 座席名称
   * 
   * @example
   * xclient
   */
  name?: string;
  /**
   * @remarks
   * 外呼挂机短信开关 0：关闭， 1：开启。开启后，当座席发起外呼时，系统会依据【短信中心 - 短信设置 - 外呼双方接听通知 / 外呼客户未接听通知】中的配置条件发送短信。
   * 
   * @example
   * 0
   */
  obHangupSms?: number;
  /**
   * @remarks
   * 是否可置忙登录，关闭后，座席不允许置忙登录，0：关闭，1：开启，默认1。
   * 
   * @example
   * 1
   */
  pauseLogin?: number;
  /**
   * @remarks
   * permission
   */
  permission?: ClinkDescribeClientResponseBodyDataClientPermission;
  /**
   * @remarks
   * 座席所属队列的队列号数组
   */
  qnos?: string[];
  /**
   * @remarks
   * 座席所属队列详情
   */
  queues?: ClinkDescribeClientResponseBodyDataClientQueues[];
  /**
   * @remarks
   * 轮选方式，1: 按天轮选；2: 按次轮选，当外显号码类型为全部、外显规则为轮选时配置才生效
   * 
   * @example
   * 1
   */
  recurrentselectionType?: number;
  /**
   * @remarks
   * 轮选值设置，当外显号码类型为全部、外显规则为轮选时配置才生效 1: 按天轮选，每 n 天外呼更换一次外显号码，可设置 1-30 天 2: 按次轮选，每 n 次外呼更换一次外显号码，可设置 1-30 次
   * 
   * @example
   * 1
   */
  recurrentselectionValue?: number;
  /**
   * @remarks
   * 座席备用手机号
   * 
   * @example
   * xxx
   */
  reserveTel?: string;
  /**
   * @remarks
   * 座席角色，0: 普通座席；1: 班长座席
   * 
   * @example
   * 1
   */
  role?: number;
  /**
   * @remarks
   * 座席服务地区，区号数组 配置座席服务的地区，可用于外呼任务中“按服务地区分配”的策略。
   */
  serveArea?: string[];
  /**
   * @remarks
   * 座席状态，0: 离线；1: 在线
   * 
   * @example
   * 1
   */
  status?: number;
  /**
   * @remarks
   * 电话绑定类型，电话类型， 1: 固话；2: 手机；3:IP话机；4:软电话
   * 
   * @example
   * 1
   */
  telType?: number;
  /**
   * @remarks
   * 座席类型，1：全渠道、2：呼叫中心、3：在线客服
   * 
   * @example
   * 1
   */
  type?: number;
  /**
   * @remarks
   * 整理时长，座席进行外呼操作后的整理时间，取值范围 0-300 秒。
   * 
   * @example
   * 59
   */
  wrapupTime?: number;
  static names(): { [key: string]: string } {
    return {
      active: 'Active',
      areaCode: 'AreaCode',
      autoInvestigation: 'AutoInvestigation',
      bindTel: 'BindTel',
      clid: 'Clid',
      clidArea: 'ClidArea',
      clidRule: 'ClidRule',
      clidType: 'ClidType',
      cloudNumberEnabled: 'CloudNumberEnabled',
      cloudNumberModes: 'CloudNumberModes',
      cno: 'Cno',
      crmId: 'CrmId',
      hiddenTel: 'HiddenTel',
      ibWrapupTime: 'IbWrapupTime',
      ibWrapupType: 'IbWrapupType',
      name: 'Name',
      obHangupSms: 'ObHangupSms',
      pauseLogin: 'PauseLogin',
      permission: 'Permission',
      qnos: 'Qnos',
      queues: 'Queues',
      recurrentselectionType: 'RecurrentselectionType',
      recurrentselectionValue: 'RecurrentselectionValue',
      reserveTel: 'ReserveTel',
      role: 'Role',
      serveArea: 'ServeArea',
      status: 'Status',
      telType: 'TelType',
      type: 'Type',
      wrapupTime: 'WrapupTime',
    };
  }

  static types(): { [key: string]: any } {
    return {
      active: 'number',
      areaCode: 'string',
      autoInvestigation: 'number',
      bindTel: 'string',
      clid: { 'type': 'array', 'itemType': 'string' },
      clidArea: { 'type': 'array', 'itemType': ClinkDescribeClientResponseBodyDataClientClidArea },
      clidRule: 'number',
      clidType: 'number',
      cloudNumberEnabled: 'number',
      cloudNumberModes: { 'type': 'array', 'itemType': 'number' },
      cno: 'string',
      crmId: 'string',
      hiddenTel: 'string',
      ibWrapupTime: 'number',
      ibWrapupType: 'number',
      name: 'string',
      obHangupSms: 'number',
      pauseLogin: 'number',
      permission: ClinkDescribeClientResponseBodyDataClientPermission,
      qnos: { 'type': 'array', 'itemType': 'string' },
      queues: { 'type': 'array', 'itemType': ClinkDescribeClientResponseBodyDataClientQueues },
      recurrentselectionType: 'number',
      recurrentselectionValue: 'number',
      reserveTel: 'string',
      role: 'number',
      serveArea: { 'type': 'array', 'itemType': 'string' },
      status: 'number',
      telType: 'number',
      type: 'number',
      wrapupTime: 'number',
    };
  }

  validate() {
    if(Array.isArray(this.clid)) {
      $dara.Model.validateArray(this.clid);
    }
    if(Array.isArray(this.clidArea)) {
      $dara.Model.validateArray(this.clidArea);
    }
    if(Array.isArray(this.cloudNumberModes)) {
      $dara.Model.validateArray(this.cloudNumberModes);
    }
    if(this.permission && typeof (this.permission as any).validate === 'function') {
      (this.permission as any).validate();
    }
    if(Array.isArray(this.qnos)) {
      $dara.Model.validateArray(this.qnos);
    }
    if(Array.isArray(this.queues)) {
      $dara.Model.validateArray(this.queues);
    }
    if(Array.isArray(this.serveArea)) {
      $dara.Model.validateArray(this.serveArea);
    }
    super.validate();
  }

  constructor(map?: { [key: string]: any }) {
    super(map);
  }
}

export class ClinkDescribeClientResponseBodyData extends $dara.Model {
  client?: ClinkDescribeClientResponseBodyDataClient;
  /**
   * @remarks
   * 请求 id
   * 
   * @example
   * 示例值示例值示例值
   */
  clinkRequestId?: string;
  static names(): { [key: string]: string } {
    return {
      client: 'Client',
      clinkRequestId: 'ClinkRequestId',
    };
  }

  static types(): { [key: string]: any } {
    return {
      client: ClinkDescribeClientResponseBodyDataClient,
      clinkRequestId: 'string',
    };
  }

  validate() {
    if(this.client && typeof (this.client as any).validate === 'function') {
      (this.client as any).validate();
    }
    super.validate();
  }

  constructor(map?: { [key: string]: any }) {
    super(map);
  }
}

export class ClinkDescribeClientResponseBody extends $dara.Model {
  accessDeniedDetail?: string;
  /**
   * @example
   * OK
   */
  code?: string;
  data?: ClinkDescribeClientResponseBodyData;
  /**
   * @example
   * OK
   */
  message?: string;
  /**
   * @example
   * ED815433-724A-4357-9991-A54AD2FF09FD
   */
  requestId?: string;
  static names(): { [key: string]: string } {
    return {
      accessDeniedDetail: 'AccessDeniedDetail',
      code: 'Code',
      data: 'Data',
      message: 'Message',
      requestId: 'RequestId',
    };
  }

  static types(): { [key: string]: any } {
    return {
      accessDeniedDetail: 'string',
      code: 'string',
      data: ClinkDescribeClientResponseBodyData,
      message: 'string',
      requestId: 'string',
    };
  }

  validate() {
    if(this.data && typeof (this.data as any).validate === 'function') {
      (this.data as any).validate();
    }
    super.validate();
  }

  constructor(map?: { [key: string]: any }) {
    super(map);
  }
}

