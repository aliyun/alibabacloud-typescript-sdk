...
export class DeleteRouteEntriesRequest extends $dara.Model {
   ...
   * 
   * **true**: performs a dry run without actually deleting the route entries. The system checks the AccessKey pair, the authorization of the Resource Access Management (RAM) user, and the required parameters. If the check fails, the corresponding error is returned. If the check passes, the `DryRunOperation` error code is returned.
   * 
   * **false** (default): sends a normal request. If the check passes, a 2xx HTTP status code is returned and the route entries are deleted.
   */
  dryRun?: boolean;
  ownerAccount?: string;
  ownerId?: number;
